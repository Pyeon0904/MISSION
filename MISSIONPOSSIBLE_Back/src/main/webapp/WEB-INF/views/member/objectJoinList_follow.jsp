...
<body>
					...
					<div class="pagination-container">
						<div class="pagination">
							<!-- 이전 페이지로 -->
							<a class="pagination-newer" href="${ path }/member/objectJoinList_follow?page=${ pageInfo.prvePage }">PREV</a>			
							<!--  10개 페이지 목록 -->
							<c:forEach begin="${ pageInfo.startPage }" end="${ pageInfo.endPage }" step="1" varStatus="status">
					            ...
					            <c:if test="${ pageInfo.currentPage != status.current}">
					               	<a href="${ path }/member/objectJoinList_follow?page=${ status.current }"><c:out value="${ status.current }"/></a>
					            </c:if>
					        </c:forEach>
					        <!-- 다음 페이지로 -->
							<a class="pagination-older" href="${ path }/member/objectJoinList_follow?page=${ pageInfo.nextPage }">NEXT</a>		
						</div><!-- #pageBar -->
