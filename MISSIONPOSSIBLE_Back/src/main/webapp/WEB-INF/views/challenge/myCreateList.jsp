				...
				<div id="container">
					<div class="inner">
						<a> 
							...
							<div style="width:200px; height:30px; position:relative; top:-70px;"><h2 style="color:black; font-size:1.5em;">개설한 챌린지</h2></div>
						</a>
								...
								<c:if test="${ list != null }">
									<c:forEach var="challenge" items="${ list }">
										<tr>
											...
											<td><fmt:formatDate type="both" value="${ challenge.createDate }" pattern="yyyy-MM-dd"/></td>
										</tr>
									</c:forEach>
								</c:if>
							</tbody>
					...
					<div style="align: center; margin-top:10px; text-align:center;">
						<!-- 맨 처음으로 -->
						<button onclick="location.href='${ path }/challenge/myCreateList?page=1'">&lt;&lt;</button>

						<!-- 이전 페이지로 -->
						<button onclick="location.href='${ path }/challenge/myCreateList?page=${  pageInfo.prvePage }'">&lt;</button>
						...
						<c:forEach begin="${ pageInfo.startPage }" end="${ pageInfo.endPage }" step="1" varStatus="status">
							...
							<c:if test="${ pageInfo.currentPage != status.current }">
								<button onclick="location.href='${ path }/challenge/myCreateList?page=${ status.current }'">
									<c:out value="${ status.current }" />
								</button>
							</c:if>
						</c:forEach>
						...
						<!-- 다음 페이지로 -->
						<button onclick="location.href='${ path }/challenge/myCreateList?page=${  pageInfo.nextPage }'">&gt;</button>

						<!-- 맨 끝으로 -->
						<button onclick="location.href='${ path }/challenge/myCreateList?page=${  pageInfo.maxPage }'">&gt;&gt;</button>
					</div>
				</div>
			</div>
		</div>
